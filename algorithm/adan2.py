...
def __parse_args__():
  ...
  parser.add_argument('-cf', '--configuration-file', help='The configuration file where the output dir, among others, are specified', type=str, required=True)

  parser.add_argument('-dsfo', '--data-source-format', help='The format for the datasource file', type=str)

  parser.add_argument('-ptoc', '--parse-to-csv', help='Do I have to parse the input datasource to CSV', action='store_true')
...
def __run_with_tensorflow__(argv):
  ...
  from pre import oral_cancer as pre
  from pos import oral_cancer as pos

  args, conf = pre.processing(args)
  ...
  while tsac < tsta or current_successful_tests < nste:
    try:
      ...
      # Fetch the data
      (df_training, train_y), (df_testing, test_y), df_predict, expected = data.read_source(args, conf)

      __print_params__(n_test, args, df_training, test_y, expected)

      df_training, features = pos.processing(df_training, args)

      df_training.to_csv('pba.csv', index=False)
      weights = []
      ...
      classifier = tf.estimator.DNNClassifier(feature_columns=features, hidden_units=hidden_units, n_classes=len(data.LABELS))
