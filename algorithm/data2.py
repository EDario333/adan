...
def __choose_random_features__(args=None, source_df=None, conf=None):
  assert args is not None, 'The args are missed'
  assert source_df is not None, 'Please specify the source dataframe'
  assert conf is not None, 'The configuration params (JSON file) are missed'
  ...
  #file_name = file_name[0:file_name.find('.')]
  output_dir = adan_utils.get_dir(conf['output_dir'])
  filename = output_dir + '/'
...
def __choose_random_data__(args=None, randomized_df=None, conf=None):
  assert args is not None, 'The args are missed'
  ...
  # The next 2 lines are for differente cases/records (a sample)
  #sstr = int(tsda * args.starting_percent_training)
  #df_training = df.sample(sstr)

  # The next 5 lines are for the same cases/records, always
  df_training = pd.DataFrame()
  for x in range(len(df)):
    if x % 3 != 0:
      df_training = df_training.append(df.iloc[x-1, :])
  sstr = len(df_training)

  file_name_ = __get_file_name_without_extension__(args)
  file_name = output_dir + '/' + file_name_ + '-' + str(ssfe) + 'features-training-data-'
  file_name += str(sstr) + '-rows.csv'
  df_training.to_csv(file_name, index=False)

  df_testing = df[~df.isin(df_training).all(1)]
  file_name = output_dir + '/' + file_name_ + '-' + str(ssfe) + 'features-test-data-'
  ...
  file_name = output_dir + '/' + file_name_ + '-' + str(ssfe) + 'features-predict-data-'
...
def read_source(args=None, conf=None):
  ...
  #randomized_df = __choose_random_features__(args, df, conf) 
  #randomized_df = pd.concat([randomized_df, df.pop(args.label)], 'columns')

  #(df_train, train_y), (df_testing, test_y), df_predict, expected = __choose_random_data__(args, randomized_df, conf)
  (df_train, train_y), (df_testing, test_y), df_predict, expected = __choose_random_data__(args, df, conf)
